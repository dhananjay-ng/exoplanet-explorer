...
gulp.task('serve', gulp.series('styles', 'elements', 'images', function () {
  browserSync({
    port: 5000,
    notify: false,
    logPrefix: 'PSK',
  ...
  gulp.watch(['app/styles/**/*.css'], gulp.series('styles'), reload);
  gulp.watch(['app/elements/**/*.css'], gulp.series('elements'), reload);
  gulp.watch(['app/{scripts,elements}/**/{*.js,*.html}'], gulp.series('jshint'));
...
gulp.task('default', gulp.series('clean', function (cb) {
  runSequence(
    gulp.series('copy', 'styles'),
    'elements',
    gulp.series('jshint', 'images', 'fonts', 'html'),
